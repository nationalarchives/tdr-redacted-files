...
object RedactedFileMatcher {
  val noOriginalFileMessage = "No Original Found"
    ...
    filesInDirectories.flatMap { directoryFiles =>
      ...
      val matchedResults: Seq[RedactedResult] = nonDuplicateRedactedFiles.map { redactedFile =>
        ...
        originalFiles match {
          ...
          case Nil => RedactedFilePairs(redactedFile.fileId, noOriginalFileMessage, redactedFile.fileId, redactedFile.filePath)
